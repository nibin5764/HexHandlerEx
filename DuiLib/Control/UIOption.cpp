...
namespace DuiLib
{
	COptionUI::COptionUI() : m_bSelected(false), m_dwSelectedBkColor(0), m_dwSelectedTextColor(0), m_SelectedFont(-1), m_SelectedUnderLineColor(0)
	{
	}
	...
	void COptionUI::SetSelectedFont(DWORD dwFont) {
		m_SelectedFont = dwFont;
	}
	DWORD COptionUI::GetSelectedFont() {
		return m_SelectedFont;
	}

	void COptionUI::SetSelectedUnderlineColor(DWORD dwColor) {
		m_SelectedUnderLineColor = dwColor;
	}
	DWORD COptionUI::GetSelectedUnderlineColor() {
		return m_SelectedUnderLineColor;
	}
	...
	void COptionUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("selectedunderlinecolor")) == 0) {
			if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
			SetSelectedUnderlineColor(clrColor);
	...
	void COptionUI::PaintStatusImage(HDC hDC)
	{
		if( (m_uButtonState & UISTATE_SELECTED) != 0 ) {
			...
			if( DrawImage(hDC, m_diSelected) ) goto Label_ForeImage;
				...
				if (m_SelectedUnderLineColor != 0) {
					RECT rcUnderLine;
					rcUnderLine.top = m_rcPaint.bottom - 5 ;
					rcUnderLine.bottom = m_rcPaint.bottom - 2;
					int rcWide = m_rcPaint.right - m_rcPaint.left;
					rcUnderLine.left = m_rcPaint.left + rcWide/10;
					rcUnderLine.right = m_rcPaint.right - rcWide / 10;
					CRenderEngine::DrawColor(hDC, rcUnderLine, GetAdjustColor(m_SelectedUnderLineColor));
				}
